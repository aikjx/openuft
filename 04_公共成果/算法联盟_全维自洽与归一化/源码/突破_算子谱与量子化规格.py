...
定理 H 已证明：**几何作用量类**（∫κ²,∫τ²,…）永远没有内点极小 ⇒ α 自由或被推到 α∈{0,∞}。
...
本引擎的判定（全部符号/数值实算，不调参）：
  ...
  2. 结论与定理 H 的关键差别：**算子类可以有内点极小**（nullity_dyn=0 ⇒ α 可被固定）！
...
L.append("| 几何作用量（旋钮零空间） | **不可行**（无内点极小 / 极小必在边界） | 定理 H |")
...
L.append("2. 但所需的调参是 **$O(\\alpha^2)$（小量）**，与定理 H 的 **$O(\\alpha^{-2})$（大量）** 定性不同；")
