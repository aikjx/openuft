...
def section_s3():
    ...
    # S3-05 定义 A 分母下的四力和（必须分别给出「同 α 口径」与「文献混 α 口径」）
    sum4_over_na_mix = tot / na_c                     # 分子 α=1/137，分母 α=CODATA（文献实际做法）
    nb_codata = 1.0 / (ALPHA * ALPHA) + 1.0 / ALPHA + 1.0 + ALPHA
    sum4_over_na_same = nb_codata / na_c              # 两端同为 CODATA α
    BO("S3-05", sec, "【新增 X13】分母取 N_A 时四力之和 ≠ 1（量级随 α 口径剧变）",
      "①同 α 口径（两端均 CODATA）：N_B/N_A=" + g(sum4_over_na_same, 12) +
      "，缺额仅 " + g(1.0 - sum4_over_na_same, 4) + "；" +
      "②文献混用口径（分子 α=1/137、分母 α=CODATA）：" + g(sum4_over_na_mix, 8) +
      "，缺额 " + g(1.0 - sum4_over_na_mix, 4) + "。" +
      "⇒ 逻辑上定义 A（无穷级数）与『四力归一化=1』**不能严格同时成立**（余量即 α²,α³,… 高阶项），" +
      "但同口径下余量只有 2.8e-9，数值上可忽略；**5.2e-4 的缺口是 α 混用造成的**（与 X1 同源）。" +
      "严重度应为『低（逻辑边界）』而非数值矛盾。")
