...
ROUTES = [
    ('h01_space_motion', '空间运行与螺旋运动', '空间运动、速度场或螺旋结构是否能够产生可观测的场？', ['螺旋', '三重奏', 'frenet', 'triad']),
    ...
    ('h04_geometry_action', '几何与作用量', '给定几何结构与作用量，能否一致导出动力学和已知极限？', ['作用量', '变分', '几何化', 'derivation']),
...
def main():
    ...
    if manifest.exists():
        ...
        for name in ['README.md', 'INDEX.md', 'WORKFLOW.md', 'QUICKSTART.md', 'MIGRATION_GUIDE.md']:
            backup = ROOT / '95_history_archive/layout_before_20260907' / name
...
def check():
    ...
    for r in data['original_files']:
        p = ROOT / ('95_history_archive/layout_before_20260907/' + r['path'] if r['path'] in replaced else r['path'])
        if not p.exists():
