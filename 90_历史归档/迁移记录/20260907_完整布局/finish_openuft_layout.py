...
with (root / '03_跨体系研究/comparison_matrix.csv').open('w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
...
for name in ['restructure_openuft_layout.py', 'finish_openuft_layout.py']:
    ...
    dst = root / '90_历史归档/迁移记录/20260907_完整布局' / name
    ...
    src.rename(dst)
manifest_path = root / '90_历史归档/迁移记录/20260907_完整布局/manifest.json'
