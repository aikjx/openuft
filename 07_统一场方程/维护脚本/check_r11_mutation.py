...
就是 R16/R17 那两台共用机器，把它们从外面改红等于测别的层 $\Rightarrow$ 五个变异体**全部只动本节自己建的
机器**（§14.10 的 $\varepsilon$ 目录枚举、$\sigma$ 线摆放与传播子、第三张票的 `g_bracket`、R18.3 的摆放
枚举），没有一例去碰 R16 的 $c_{j0}$ 递推或 14.8 那台零空间基 $\Rightarrow$ 而这正是"票与票不共享算术"这句
话可证伪的形式：改错一张，另两张照旧。
**(a)** §14.10 的 $\varepsilon$ 目录只在不带点侧枚举（同侧那一对带点缩并整批不生成）$\Rightarrow$ 两个前置
读数同时红：表格档"四数**不全等**"从 0 涨到 6 档（R18.0），"$\sigma$ 道并进之后的**新增结构**"从 0 读到
370 条（R18.1）$\Rightarrow$ 本层的"完备"读数是这两句话一起说的，缺一句就把差额当成了恒等。
**(b)** 把带点侧槽位的起点从 $\nu$ 写回 $0$（$\varepsilon$ 目录从此跨了手性）$\Rightarrow$ **这一条复现本层
写作时真踩过的伤**：六个表格档里只有 $(4,4)$ 一档动（$\varepsilon$ 秩、$\sigma$ 单独秩、并秩一起
$4\to3$，$\sigma$ 去重 $36\to24$、道间关系 $41\to30$），"不全等"档数 $0\to1$，而"新增结构"**照样读 0**
$\Rightarrow$ 保护这条读数的是"目录秩 $=$ 预测"那面前置等式而不是"新增 $=0$"自己，本层因此把两件事写进
同一条门禁（R18.1）$\Rightarrow$ 这句话从此有了一次实测反例（红的只有 R18.0）。**(c)** $\sigma$ 线传播子
把点号侧的两个指标接反一位（$P_{ij,pq}$ 读成 $P_{ij,qp}$ —— 度规留着、只接错一头）
$\Rightarrow$ 两条教科书锚点互换符号（$2/-2\to-2/2$，R18.0 把这两格钉成常数），表格档"不全等"仍读 0 档
$\Rightarrow$ 抓它的是锚点那一对**读数**，不是那条完备性判据。**(d)** `g_bracket` 丢掉
"基元素按生成元**升序**落定"那一次的换序符号（`F(e if g1 < g2 else -e)` 写成 `F(e)`）$\Rightarrow$ 同名与
异名两条括号一起归零（报告里那一栏读成空），第三张票（Grassmann 道）在 14.9 那 838 个类上逐类相乘后合计
从 838 读到 **619**、逐类不等 **219** 类（R18.2 的 $838=838$ 红），管辖范围那条和式同时红（R18.4：单侧场
计数与判定类数不再相加等于 838）$\Rightarrow$ 与 R17 的 l1 是**同一处伤在新坐标里的复现**，钉住它的是
R18.0 的锚点、R18.2 那个 $838=838$ 与 R18.4 的和式。**(e)** 摆放不变性只在
...
三条负面结果如实登记（与 R13.2 同族，只记在这段散文里、不进台账的例数）：**(i)** 共轭签名的相位
...
列只核对数量、不核对它挂在哪一半上。**(iii)** 是 R18 这一层新添的、也是本层最贵的一条：把 §14.10 的
$\sigma$ 传播子 $P=\sum_\mu G_\mu\sigma^\mu\sigma^\mu$ 去掉度规号 $G=(+1,-1,+1,-1)$（只乘两份
$\sigma$ 分量）$\Rightarrow$ 底层的读数**确实动了**（表格档"四数不全等"从 0 档读到 6 档），但度规一掉，
同向与交叉两条 $\sigma$ 线和 $\varepsilon$ 目录之间不再有公共比值，锚点比值的渲染器 `_rt` 从此返回
None $\Rightarrow$ None 落进正文就撞上报告自己的**排版不变量 #2**（`write_report` 里"正文不许漏未格式化
的 Python 对象"那条 assert），它在任何一条门禁判决写到 stdout **之前**、也在报告写盘之前退出 $\Rightarrow$
台账按"崩溃不算捕获"记它 MISS（第一次全量重跑正是这样收场：`verdict` INVALID、`missed` 里就是这一例）。
换成 (a) 那处植入（$\varepsilon$ 目录只枚举不点侧）就两头都有读数、报告照常出，于是它进台账、这一例
只进这段散文。教训分两头：**(i)** 的教训是位置——能植红的位置必须下游有读数，而**渲染层也算下游**，
一条度量号错误可以只被排版不变量抓住，这时"抓到它"的不是任何一条数值判据；**(iii)** 因此是本层
"四张票不共享算术"这句话的**仪器边界**：五例里只有 (c) 一例动的是 $\sigma$ 传播子本身，度量号的错
...
# 四张票里的预测路（cg_j0）与 14.8 那台显式零空间机器都是前面各层共用的
# ⇒ 从外面改它们测不到本节这一张票（同 R16/R17 的甲路边界）⇒ 五例动的都是**本节自己建的机器**
# （§14.10 的 $\varepsilon$ 目录枚举、$\sigma$ 线摆放与传播子、第三张票的 `g_bracket`、R18.3 的摆放枚举），
# 而"票与票不共享算术"这句话的可证伪形式正是：改错一张、另两张照旧。
# 曾试过的"传播子去掉度规号"这一处在本层植不红（它只被报告的排版不变量 #2 抓住 ⇒ 崩溃不算捕获），
# 已按负面结果登记在 docstring 的 (iii)，不在下面五例之内。
M_EUND = (b'                el.append((list(pu) + list(pd), [], []))',
          b'                el.append((list(pu), [], []))')
...
CASES = [
    ...
    ('n1', 'R18：§14.10 的 eps 目录只枚举不带点侧（同侧那一对带点缩并整批不生成 ⇒ 表格档"不全等"0→6 档、'
           '"sigma 道并进后的新增结构"0→370 条：完备性这两半一起红，R18.0 与 R18.1）', 'mutant', M_EUND, 'R18'),
    ('n2', 'R18：带点侧槽位的起点从 nu 写回 0（eps 目录从此跨手性 ⇒ 六个表格档里只有 (4,4) 一档动：三张秩'
           '4→3、sigma 去重 36→24、道间关系 41→30，而"新增结构"**照样读 0**：复现本层写作时真踩过的伤，'
           '钉住它的只有 R18.0 那个前置等式）', 'mutant', M_SIDE, 'R18'),
    ('n3', 'R18：sigma 线把点号侧的两个指标接反一位（P_{ij,pq} 读成 P_{ij,qp}，度规留着只接错一头'
           '⇒ 动的只有两条教科书锚点（互换符号 2/-2 → -2/2），表格档"不全等"仍读 0 ⇒ 抓它的是锚点那一对'
           '钉住的读数，不是那条完备性判据）', 'mutant', M_PIDX, 'R18'),
    ('n4', 'R18：g_bracket 丢掉"基元素按生成元升序落定"那一次的换序符号（同名与异名两条括号一起归零 ⇒ 第三'
           '张票在 14.9 那 838 个类上逐类相乘后合计 838→619、逐类不等 219 类，R18.0/18.2/18.4 一起红：'
           '与 R17 的 l1 是同一处伤在新坐标里的复现）', 'mutant', M_GSIG, 'R18'),
