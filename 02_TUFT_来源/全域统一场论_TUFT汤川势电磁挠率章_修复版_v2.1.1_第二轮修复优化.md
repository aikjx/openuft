...
> **版本**：v2.1.1（2026-09-26）｜**上游**：v2.0/修复版（R1–R6，《全域统一场论_TUFT拓扑统一场论_汤川势与电磁挠率章_修复版.md》）+ 算法联盟总报告 + 全维校验脚本
...
**v2.0 隐式假设 $V(\infty)=0$，没有显式声明。**
- 数学上 $E_1(\infty)=0$，满足远场势归零；
- 但指数积分 $E_1(0^+)\to+\infty$，$r\to0$ 时 $V\to V(\infty)-c^2A(-\gamma-\ln\mu r)$；
- 若不写明 $V(\infty)=0$，在 **TOV 耦合时会出现势常数偏移**，EoS 能量基准漂移，造成中子星总内能系统偏差（正是 v2.0 → v2.1.1 中子星多信使拟合中的一个隐藏次要偏差源）。
...
$\kappa,\tau$ 为曲率、Frenet 挠率，量纲 $\mathrm{m}^{-1}$；$\sqrt{\kappa^2+\tau^2} \Rightarrow \mathrm{m}^{-1}$；$\hbar/c$ 量纲 $\mathrm{kg\cdot m}$，乘积 ⇒ $\mathrm{kg}$。**量纲完全匹配 ✔**；但 v2.0 没有在正文显式给出量纲校验块。
...
仿真输出：$r=10^{-16}\text{m}$，$u_\tau \sim 10^{80}\ \text{J/m}^3$。审计：这是**无量纲标定 $C_\tau=1,\mathcal{E}_\tau=1$ 带来的人工量级，不是真实物理能量密度**。v2.0 正文缺少强警告，容易误读为真实真空场能量。
...
球对称解：$\kappa(r)=\dfrac{A}{r}e^{-\mu r}$；点源通量极限 $\lim_{r\to0}4\pi r^2 \partial_r \kappa = -4\pi A$，匹配右侧 $-4\pi C_\kappa$，得 $A=C_\kappa$。数学正确；但歧义：这个通量是**曲率场通量**，不是引力通量或电磁通量。v2.0 没有标注"曲率通量"。
...
v2.0 原文：$\varepsilon_0,e,\alpha$ 不可由三公理导出。补充审计：TUFT 公理只确定**场的微分方程结构、几何映射关系**，属于**动力学结构公理**；不包含离散量子电荷量子化条件、精细结构常数的几何量子化边界。$\alpha$ 是量子孤子自能的量子化能级比值，属于孤子边界条件，不是连续场微分方程的必然推论。
...
| 本版条目 | v2.0/修复版 对应 | 关系 |
|---|---|---|
| R7 V(∞)=0 | v2.0 §1.3 已写 $V=-c^2A\,E_1(\mu r)$ 但未显式边界 | 补显式边界声明 + 脚本远场残差 |
| R8 κ_X vs κ_int | v2.0 §1.1/§1.4 符号同名不同载体 | 正文加区分说明 + 量纲块 |
| R9 A^μ=kτ^EM,μ | v2.0 §2.2 映射 | 四矢势规范对齐，静态代码不变 |
| R10 无量纲警告 | v2.0 §3 缺警告 | §3 新增 ⚠️ 块 |
| R11 曲率场通量 | v2.0 §1.2 未注明 | 注明曲率场高斯通量 |
| R12 O15 细化 | v2.0 §四 O15 | 描述精细化，仍 🔴 |
