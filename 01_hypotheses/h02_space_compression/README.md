# H02_SPACE_COMPRESSION · 空间压缩与密度梯度

研究问题：空间压缩量如何定义、测量，并与质量、引力及场方程联系？

状态：待建模／待审查。下列历史关联材料尚未逐项核实，不能据此宣布该路线成立。

- [0：假设与边界](00_assumptions/README.md)
- [1：数学定义与量纲](01_definitions/README.md)
- [2：方程推导](02_derivations/README.md)
- [3：数学证明与一致性](03_proofs/README.md)
- [4：可区分预测](04_predictions/README.md)
- [5：复现与实验对照](05_verification/README.md)
- [6：反例与证伪](06_falsification/README.md)
- [7：结论与未解问题](07_conclusions/README.md)

[候选资料索引](sources.md) · [跨体系比较](../../02_comparison/README.md) · [研究记录模板](../../03_research_protocol/claim_template.md)
