...
def main():
    ...
    L.append("")
    # 第八章：判据门禁（自动）
    gate_path = os.path.join(HERE, "tuft_判据门禁.json")
    if os.path.isfile(gate_path):
        g = json.load(open(gate_path, encoding="utf-8"))
        L.append("### 7.1 判据门禁（自动）")
        L.append("")
        L.append("门禁：`tuft_判据门禁.py`（量纲 / 可观测锚 / 重述 / 代码 四道）。")
        L.append("")
        L.append("**量纲非法 %d 条**（实算 vs 期望，精确有理指数）：" % g.get("dim_fail", 0))
        L.append("")
        for row in g.get("dimension", []):
            if row["verdict"] == "FAIL":
                L.append("- %s → 实算 `%s`，应为 `%s`" % (row["expr"], row["got"], row["expect"]))
        L.append("")
        L.append("- 无观测锚（不可判决）分册：%s" % ("、".join(g.get("obs_anchor_missing", [])) or "无 ✅"))
        L.append("- PASS 全为\"重述/继承/标准\"的分册：%s" % ("、".join(g.get("restate_only", [])) or "无 ✅"))
        L.append("- 声称仿真却未逐行复现的分册：%s" % ("、".join(g.get("code_no_replay", [])) or "无 ✅"))
        L.append("")

    L.append("- 编排总览：`tuft_全景总报告.md`；自动索引：`tuft_总索引.md`；缺陷族：`tuft_跨册缺陷族.md`；门禁：`tuft_判据门禁.md`")
