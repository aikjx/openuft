...
> **关键事实**：EDM 预言 $2.26\times10^{-34}\,\mathrm{C\cdot m}$。⚠️ **本章原稿"低于实验上限 $8.7\times10^{-34}\,\mathrm{C\cdot m}$、仍被允许"的结论已撤销**——经第十编第53章核证，该上限数值有误（详见 §48.3 勘误块）。**正确裁定：预言已被 JILA HfF⁺ 2023 实验排除 16.5 个数量级，EDM 通道关闭。** g−2 未解决，反而引入新矛盾。α 是独立输入参数。
...
- ~~实验上限（90% CL）$d_e<8.7\times10^{-34}\,\mathrm{C\cdot m}$~~　**⚠️ 该上限值有误，见下方勘误块。**

---

### ⚠️ 勘误块（第十编第53章裁定，以本章为准的修订）

**原判定（已撤销）**：

$$\cancel{2.26\times10^{-34}<8.7\times10^{-34}\ \Longrightarrow\ \text{ZUFT 预测在允许范围内}}$$

**错误所在**：$8.7\times10^{-34}\,\mathrm{C\cdot m}$ 这一"实验上限"与权威值相差约 **17 个数量级**。

单位换算核对（$1\ \mathrm{e\cdot cm}=1.602\times10^{-21}\ \mathrm{C\cdot m}$）：

| 量 | e·cm | C·m | 来源 |
|---|---|---|---|
| JILA HfF⁺ 上限（2023，世界纪录） | $4.1\times10^{-30}$ | **$6.569\times10^{-51}$** | Science 2023, adg4084 |
| ACME II 上限（2018） | $1.1\times10^{-29}$ | $1.762\times10^{-50}$ | Nature 562 (2018) 355 |
| 本章原稿所用"上限" | — | ~~$8.7\times10^{-34}$~~ | **来源不明，与上述相差 $\sim10^{17}$ 倍** |
| ZUFT 预言 | $1.409\times10^{-13}$ | $2.257\times10^{-34}$ | 本章 §48.2 |

**修正后的判定**：

$$\frac{d_e^{\rm pred}}{|d_e|_{\rm JILA}}=\frac{1.409\times10^{-13}}{4.1\times10^{-30}}=3.44\times10^{16}$$

> **ZUFT 电子 EDM 预言被实验排除 $3.44\times10^{16}$ 倍（16.5 个数量级）。EDM 通道关闭。**

**疑似成因**：本体系原稿可能把某一以 $\mathrm{e\cdot cm}$ 为单位的上限值（如 ACME I 2014 的 $8.7\times10^{-29}\ \mathrm{e\cdot cm}$）在换算为 $\mathrm{C\cdot m}$ 时**只保留了尾数 8.7 而写错了指数与单位**，导致上限被放大约 $10^{17}$ 倍，从而使一个本应被排除的预言看起来"仍在允许范围内"。

> **这是本书迄今记录的最严重的一处数值错误：它不是一个偏差，而是把"已被排除"读作"仍被允许"，即把证伪读成了验证。**

---

**判死标准（已触发）** [A]：原定判死线 $d_e<10^{-36}\,\mathrm{C\cdot m}$。实际实验上限已达 $6.569\times10^{-51}\,\mathrm{C\cdot m}$，**远低于判死线 ⇒ 判死条件早已满足，ZUFT 螺旋 EDM 模型（$d_e=e\alpha\rho/2$）已被排除。**

**意义的重新表述** [A]：它仍然是 ZUFT **第一个具体、可检验、独立于标准物理的定量预言**——但它的结局是**被证伪**，而非被证实。这一区分至关重要：

- 作为"可检验预言"：成立，且是本书唯一走完"预言 → 实验比对 → 裁定"全程的项。
- 作为对该模型的支持：**不成立**。预言值比实验上限大 16.5 个数量级。

**修正方向** [B]：回归螺旋对称性 $d_e=0$（与标准模型一致，对称性论证 [A]）；放弃 $g2\_EDM.py$ 的"电荷沿 $b$ 方向固定偏移"假设（该 [C] 类假设已被实验否决）。
...
| $d_e^{\rm exp}$ | 实验上限 | 90% CL | $\ell^0$ | $<6.569\times10^{-51}\,\mathrm{C\cdot m}$（$=4.1\times10^{-30}$ e·cm） | [A] Science 2023（⚠️ 原稿 $8.7\times10^{-34}$ 有误，见 §48.3 勘误块） |
...
1. **EDM 是 ZUFT 唯一可检验独立预言，且已被证伪** [A]：$d_e=2.26\times10^{-34}\,\mathrm{C\cdot m}=1.409\times10^{-13}\ \mathrm{e\cdot cm}$，比 JILA HfF⁺ 2023 上限 $4.1\times10^{-30}\ \mathrm{e\cdot cm}$ 大 **$3.44\times10^{16}$ 倍（16.5 个数量级）**。⚠️ **原稿"低于实验上限 $8.7\times10^{-34}$、仍被允许"的结论已撤销**（上限数值有误，见 §48.3 勘误块）。判死线 $d_e<10^{-36}$ **早已被实验越过**。**状态：已证伪，不再是活跃预言。**
