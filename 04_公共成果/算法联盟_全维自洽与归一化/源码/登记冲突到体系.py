...
RECORD_HDU = "11_证伪与反例/HDU紧致化半径公式缺陷记录.md"
RECORD_TCL = "11_证伪与反例/TCL代数维数错误记录.md"
...
CLAIMS = {
    ...
    # ── O 级体系隔离审查发现的、独立于 M02 谱系的真实缺陷（2026-09-15）──
    "S05_HDU高维紧致化统一": [
        ("S05-C0001",
         "【缺陷】定理 H2 紧致化半径公式 R11=(hbar/2pi)^(1/3)·G^(1/3)·c^(-2/3) 量纲为 L·T^(-1/3)（非长度）"
         "；实算 2.318570e-21 m 与 Planck 长度 1.616255e-35 m 相差 1.4345e14 倍"
         "——与同文精算表 H1『误差<1e-4』直接矛盾（独立于 M02 谱系）",
         "S05-A2;定理 H2", RECORD_HDU,
         "隔离审查：量纲 m·s^(-1/3)；R11/L_p = 1.4345e+14"),
        ("S05-C0002",
         "【缺陷】定理 H3 数值错误：M11 = M_p·(2pi)^(1/3) 实算 2.2529e19 GeV"
         "；原文声称 3.18e19 GeV（偏大 1.412 倍）；公式形式正确但数值与自身声明不符",
         "S05-A1;定理 H3", RECORD_HDU,
         "隔离审查：M11 实算 2.2529e+19 GeV vs 原文 3.18e19 GeV"),
    ],
    "S06_TCL拓扑手征锁定": [
        ("S06-C0001",
         "【缺陷】定义 4 代数同构错误：Cl(4-4)⊗C 应 ≅ M_16(C)（维数 256）"
         "；原文写作 M_8(C)⊕M_8(C)（维数 128）；复数 Clifford 代数为单代数不能是半单的 M_8⊕M_8",
         "S06-A1;定义 4", RECORD_TCL,
         "隔离审查：M_16(C) dim=256 ≠ M_8⊕M_8 dim=128"),
        ("S06-C0002",
         "【缺陷】精算表 T1 维数自相矛盾：原文『|Cl(4-4)| 维 = 2^4·2 = 32』无依据"
         "；实为 2^8=256；与同文定义 4 的 128 亦互斥（32/128/256 三者不能同时成立）",
         "S06-A1", RECORD_TCL,
         "隔离审查：2^4·2=32 无依据；实为 2^8=256"),
    ],
}
...
README_TAIL = {
    ...
    "S05_HDU高维紧致化统一":
        "本阶段产物：[HDU 紧致化半径公式缺陷记录](HDU紧致化半径公式缺陷记录.md)"
        "（S05-C0001 量纲错误且偏离 1.4e14 倍；S05-C0002 数值错误；均为隔离审查发现的独立缺陷）。",
    "S06_TCL拓扑手征锁定":
        "本阶段产物：[TCL 代数维数错误记录](TCL代数维数错误记录.md)"
        "（S06-C0001 代数同构错误应为 M_16(C)；S06-C0002 维数 32/128/256 自相矛盾）。",
}
