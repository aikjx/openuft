# TUFT 全景索引

> 自动扫描生成（`tuft_总索引.py`）。红线：数学自洽 != 实验证实。

## 一、报告判定统计汇总

| 报告 | PASS | FAIL | BOUNDARY | INFO |
|---|---|---|---|---|
| tuft_B_UV完成_report.txt | 1 | 1 | 1 | 11 |
| tuft_B_排查_线性化_report.txt | 5 | 5 | 1 | 7 |
| tuft_B_根因溯源_report.txt | 8 | 1 | 1 | 2 |
| tuft_B_自屏蔽_数值求解_report.txt | 2 | 2 | 1 | 18 |
| tuft_D2_report.txt | 5 | 9 | 0 | 19 |
| tuft_D3_尺度与K_sat_锚定_report.txt | 1 | 3 | 0 | 4 |
| tuft_O_SCALE_锚定方案_report.txt | 7 | 1 | 1 | 4 |
| tuft_Q量子A_report.txt | 12 | 5 | 4 | 5 |
| tuft_Q量子B_report.txt | 1 | 25 | 3 | 5 |
| tuft_Q量子_report.txt | 10 | 30 | 5 | 11 |
| tuft_fermion_spin_report.txt | 0 | 0 | 0 | 0 |
| tuft_knot_slsqp_report.txt | 3 | 0 | 0 | 1 |
| tuft_r10_report.txt | 19 | 2 | 4 | 3 |
| tuft_r11_report.txt | 11 | 4 | 2 | 5 |
| tuft_r12_report.txt | 6 | 2 | 2 | 4 |
| tuft_r13_report.txt | 11 | 5 | 4 | 3 |
| tuft_r14_report.txt | 3 | 7 | 2 | 4 |
| tuft_r15_report.txt | 3 | 2 | 3 | 5 |
| tuft_r16_report.txt | 4 | 1 | 2 | 5 |
| tuft_r17_report.txt | 4 | 1 | 2 | 6 |
| tuft_r18_report.txt | 2 | 1 | 2 | 4 |
| tuft_r19_report.txt | 12 | 1 | 3 | 10 |
| tuft_r1_report.txt | 43 | 0 | 0 | 5 |
| tuft_r20_report.txt | 3 | 2 | 5 | 17 |
| tuft_r21_report.txt | 2 | 0 | 6 | 16 |
| tuft_r22_report.txt | 4 | 0 | 4 | 12 |
| tuft_r23_report.txt | 3 | 1 | 6 | 19 |
| tuft_r24_report.txt | 6 | 0 | 4 | 20 |
| tuft_r25_report.txt | 6 | 0 | 5 | 21 |
| tuft_r26_report.txt | 2 | 2 | 10 | 24 |
| tuft_r27_report.txt | 4 | 0 | 5 | 11 |
| tuft_r28_report.txt | 2 | 0 | 7 | 14 |
| tuft_r3_report.txt | 8 | 0 | 0 | 0 |
| tuft_r4_report.txt | 9 | 3 | 0 | 12 |
| tuft_r5_report.txt | 11 | 2 | 15 | 10 |
| tuft_r6_report.txt | 1 | 5 | 0 | 12 |
| tuft_r7_report.txt | 7 | 1 | 0 | 4 |
| tuft_r8_report.txt | 6 | 1 | 0 | 5 |
| tuft_r9_report.txt | 7 | 1 | 0 | 4 |
| tuft_三路线_ABC_report.txt | 4 | 7 | 4 | 20 |
| tuft_四力统一_report.txt | 17 | 19 | 9 | 14 |
| tuft_引力波_挠率扰动_report.txt | 4 | 0 | 0 | 8 |
| tuft_收口_修复优化_report.txt | 6 | 3 | 3 | 6 |
| tuft_暴胀CMB_report.txt | 5 | 35 | 8 | 10 |
| tuft_机电对偶_report.txt | 39 | 0 | 9 | 1 |
| tuft_机电对偶_实验对标_report.txt | 18 | 0 | 6 | 4 |
| tuft_相位pi_全维求导精算_report.txt | 6 | 9 | 6 | 7 |
| tuft_相位pi_闭合莫比乌斯_report.txt | 7 | 1 | 0 | 1 |
| tuft_续篇_全维求导精算_report.txt | 17 | 5 | 4 | 11 |
| tuft_续篇_双结交换仿真_report.txt | 5 | 0 | 0 | 6 |
| tuft_色挠率_SU3_report.txt | 5 | 16 | 6 | 14 |
| tuft_黑洞热力学_report.txt | 13 | 23 | 5 | 14 |
| **合计** | **400** | **244** | **170** | **458** |

## 二、脚本清单（71）

- `_dbg_wall.py`
- `_scratch_r28.py`
- `_test_wall_fin.py`
- `_test_wall_fin2.py`
- `_test_wall_forward.py`
- `_test_wall_jost.py`
- `_test_wall_series.py`
- `_test_wall_true.py`
- `_validate_wall_method.py`
- `r5_torsion_sim.py`
- `tuft_B_UV完成_抑制检验.py`
- `tuft_B_排查_线性化质量项.py`
- `tuft_B_根因溯源_exp来源.py`
- `tuft_B_自屏蔽_数值求解.py`
- `tuft_D2_挠率动力学_可行性判定.py`
- `tuft_D3_尺度与K_sat_锚定.py`
- `tuft_O_SCALE_锚定方案设计.py`
- `tuft_Q量子A_拓扑荷重定义.py`
- `tuft_Q量子B_非微扰拓扑扇区.py`
- `tuft_Q量子_全维求导精算.py`
- `tuft_fermion_spin.py`
- `tuft_knot_slsqp.py`
- `tuft_r10_反常消除与链环CS对偶.py`
- `tuft_r11_大统一求导证明与耦合拓扑约束.py`
- `tuft_r12_大统一缺口反解与新态预言.py`
- `tuft_r13_机电对偶与本源谐振结构.py`
- `tuft_r14_结激发规范量子数决定性检验.py`
- `tuft_r15_结群表示论补救构造.py`
- `tuft_r16_三叶结SU3表示构造.py`
- `tuft_r17_大统一缺口闭合_规范量子数与代.py`
- `tuft_r18_三代拓扑起源跨套闭合.py`
- `tuft_r19_O_KNOTMAP_构造性收窄.py`
- `tuft_r1_sim.py`
- `tuft_r20_sigma_abs0_ringdown.py`
- `tuft_r21_sigma_abs0_ringdown_timedomain.py`
- `tuft_r22_sigma_abs0_echo_comb.py`
- `tuft_r23_chebyshev_qnm_precision.py`
- `tuft_r24_matrix_pencil_qnm.py`
- `tuft_r25_leaver_qnm.py`
- `tuft_r26_leaver_qnm_full.py`
- `tuft_r26_leaver_wall_qnm.py`
- `tuft_r27_rw_zerilli_susy.py`
- `tuft_r28_kerr_teukolsky_probe.py`
- `tuft_r2_derivative_proof.py`
- `tuft_r3_scale_degeneracy.py`
- `tuft_r4_scale_generation.py`
- `tuft_r5_torsion_probe_audit.py`
- `tuft_r6_torsion_generation.py`
- `tuft_r7_拓扑UFT_链条精算.py`
- `tuft_r8_红移修复_求导证明与归一化.py`
- `tuft_r9_particle_derivation.py`
- `tuft_v4_line34_quintessence_lambda_n.py`
- `tuft_三路线_ABC_全维分析修复优化.py`
- `tuft_全书构建.py`
- `tuft_全维度总验证.py`
- `tuft_判据门禁.py`
- `tuft_四力统一_全维求导精算.py`
- `tuft_引力波_挠率扰动.py`
- `tuft_总索引.py`
- `tuft_收口_修复优化方案.py`
- `tuft_暴胀CMB_全维求导精算.py`
- `tuft_机电对偶_实验对标与压电实现.py`
- `tuft_机电对偶_谐振同构_全维精算.py`
- `tuft_相位pi_全维求导精算.py`
- `tuft_相位pi_闭合莫比乌斯验证.py`
- `tuft_第一性归一化总览.py`
- `tuft_续篇_全维求导精算.py`
- `tuft_续篇_双结交换仿真.py`
- `tuft_色挠率_SU3_全维求导精算.py`
- `tuft_跨册缺陷族检查.py`
- `tuft_黑洞热力学_全维求导精算.py`

## 三、报告文档清单（59）

- `TUFT_微观粒子拓扑导出_第十一部分.md`
- `TUFT拓扑统一场论_第四至十部分_完整整理.md`
- `tuft_B_UV完成报告.md`
- `tuft_B_根因溯源报告.md`
- `tuft_B_自屏蔽_数值求解报告.md`
- `tuft_D2_挠率动力学可行性报告.md`
- `tuft_LCS_链环陈西蒙斯对偶_文稿.md`
- `tuft_O_SCALE_锚定方案报告.md`
- `tuft_Q量子A_拓扑荷重定义报告.md`
- `tuft_Q量子_文稿_修订版.md`
- `tuft_Q量子_精算报告.md`
- `tuft_R11_大统一求导证明_文稿.md`
- `tuft_R12_大统一缺口反解_文稿.md`
- `tuft_R13_机电对偶与本源谐振_文稿.md`
- `tuft_R14_结激发规范量子数检验_文稿.md`
- `tuft_R15_结群表示论补救构造_文稿.md`
- `tuft_R16_三叶结SU3表示构造_文稿.md`
- `tuft_R17_大统一缺口闭合_规范量子数与代_文稿.md`
- `tuft_R18_三代拓扑起源跨套闭合_文稿.md`
- `tuft_R19_O_KNOTMAP_构造性收窄_文稿.md`
- `tuft_R20_sigma_abs0_ringdown_文稿.md`
- `tuft_R21_sigma_abs0_ringdown_timedomain_文稿.md`
- `tuft_R22_sigma_abs0_echo_comb_文稿.md`
- `tuft_R23_Chebyshev_QNM_方法论审计_文稿.md`
- `tuft_R25_Leaver连分式精确QNM_文稿.md`
- `tuft_R26_QNM全维验证_文稿.md`
- `tuft_R27_RW_Zerilli等谱_SUSY_文稿.md`
- `tuft_R28_Kerr_Teukolsky结构探测_文稿.md`
- `tuft_r2_全维求导证明验证精算报告.md`
- `tuft_r5_挠率探测实验_审计与修订.md`
- `tuft_三路线_ABC_全维分析修复优化报告.md`
- `tuft_全书_交付版.md`
- `tuft_全景总报告.md`
- `tuft_全维分析续篇_自旋统计泡利手性.md`
- `tuft_全维度验证总报告.md`
- `tuft_关联分析_归一化.md`
- `tuft_判据门禁.md`
- `tuft_四力统一_文稿_修订版.md`
- `tuft_四力统一_精算报告.md`
- `tuft_总索引.md`
- `tuft_排查修复报告.md`
- `tuft_收口_修复优化方案报告.md`
- `tuft_暴胀CMB_文稿_修订版.md`
- `tuft_暴胀CMB_精算报告.md`
- `tuft_机电对偶_实验对标与压电实现_文稿.md`
- `tuft_机电对偶_谐振同构_文稿.md`
- `tuft_相位pi_全维求导精算报告.md`
- `tuft_相位pi_文稿_修订版.md`
- `tuft_第一性归一化总览.md`
- `tuft_续篇B_挠率引力波.md`
- `tuft_续篇_全维求导精算报告.md`
- `tuft_色挠率_SU3_文稿_修订版.md`
- `tuft_色挠率_SU3_精算报告.md`
- `tuft_范式哲学重构.md`
- `tuft_跨册缺陷族.md`
- `tuft_黑洞热力学_文稿_修订版.md`
- `tuft_黑洞热力学_精算报告.md`
- `挠率产生机制_阶段0_可行性判定.md`
- `挠率探测实验设计_修复版.md`

## 四、原始报告（txt，52）

- `tuft_B_UV完成_report.txt`
- `tuft_B_排查_线性化_report.txt`
- `tuft_B_根因溯源_report.txt`
- `tuft_B_自屏蔽_数值求解_report.txt`
- `tuft_D2_report.txt`
- `tuft_D3_尺度与K_sat_锚定_report.txt`
- `tuft_O_SCALE_锚定方案_report.txt`
- `tuft_Q量子A_report.txt`
- `tuft_Q量子B_report.txt`
- `tuft_Q量子_report.txt`
- `tuft_fermion_spin_report.txt`
- `tuft_knot_slsqp_report.txt`
- `tuft_r10_report.txt`
- `tuft_r11_report.txt`
- `tuft_r12_report.txt`
- `tuft_r13_report.txt`
- `tuft_r14_report.txt`
- `tuft_r15_report.txt`
- `tuft_r16_report.txt`
- `tuft_r17_report.txt`
- `tuft_r18_report.txt`
- `tuft_r19_report.txt`
- `tuft_r1_report.txt`
- `tuft_r20_report.txt`
- `tuft_r21_report.txt`
- `tuft_r22_report.txt`
- `tuft_r23_report.txt`
- `tuft_r24_report.txt`
- `tuft_r25_report.txt`
- `tuft_r26_report.txt`
- `tuft_r27_report.txt`
- `tuft_r28_report.txt`
- `tuft_r3_report.txt`
- `tuft_r4_report.txt`
- `tuft_r5_report.txt`
- `tuft_r6_report.txt`
- `tuft_r7_report.txt`
- `tuft_r8_report.txt`
- `tuft_r9_report.txt`
- `tuft_三路线_ABC_report.txt`
- `tuft_四力统一_report.txt`
- `tuft_引力波_挠率扰动_report.txt`
- `tuft_收口_修复优化_report.txt`
- `tuft_暴胀CMB_report.txt`
- `tuft_机电对偶_report.txt`
- `tuft_机电对偶_实验对标_report.txt`
- `tuft_相位pi_全维求导精算_report.txt`
- `tuft_相位pi_闭合莫比乌斯_report.txt`
- `tuft_续篇_全维求导精算_report.txt`
- `tuft_续篇_双结交换仿真_report.txt`
- `tuft_色挠率_SU3_report.txt`
- `tuft_黑洞热力学_report.txt`

