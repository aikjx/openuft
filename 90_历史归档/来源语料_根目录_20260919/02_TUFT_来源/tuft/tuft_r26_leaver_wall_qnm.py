...
def main():
    ...
    # ── C1 反射壁墙零（Leaver 级数在墙处求值）：方法局限诚实标注 ──
    I_("§C1 σ_abs=0 墙零（Leaver 无穷远出波级数 f_out 在墙 x_s=1−2M/r_s≈0.02439 处求值 Σ a_n x_s^n）："
       "x_s 极小 ⇒ 级数 x_s^n 过早收敛，S 被 a_0=1 主导、对 ω 极不敏感（已验证 ω=0.41−0.03j 处 S≈1.0003 且不随 N 变），"
       "故该法【不能】用于在墙处定位反共振/腔模零点——它本就编码无穷远渐近、墙处是其『安全区』。真判据见 §C5（f_in 入波反向射击）。")
    r_s = float(RS)
    x_s = float(1.0 - 2.0 / r_s)
    N_SER = 3000
    w_ref = 0.409880 - 0.029576j
    s_ref = wall_series(w_ref, x_s, N_SER, ga, gb, gc)
    I_("参考 ω=R24 估计 %.6f%+.6fi 处 |Σ a_n x_s^n|=%.3e（≈1，不收敛到零 ⇒ 级数法在墙处失效，牛顿精修会因导数≈0 爆炸为 nan）" % (w_ref.real, w_ref.imag, abs(s_ref)))
    B_("C1 墙零级数法在 x_s≈0.024 处过早收敛于 |S|≈1、对 ω 不敏感，无法定位零点；初版据此判『腔模 PASS』是方法误用。"
       "正确墙零/腔模判据是 §C5 的 f_in(r_s)=0 入波反向射击（tortoise 坐标墙 u_s≈−5.328 处得对 ω 敏感的解）。")
    w0_w = w_ref  # 反射壁反共振参考频率（级数法失效，用 R24 时域估计值标注 τ 比）
