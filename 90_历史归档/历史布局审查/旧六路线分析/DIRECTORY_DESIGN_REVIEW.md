...
| 跨模型材料可共用 | `02_共享基础` 与显式依赖 | H01/H04 声明共享引擎；没有自动依赖图校验 |
| 比较不同模型时保留其前提 | `03_跨体系研究` 和比较登记表 | 字段存在；公平比较尚需真实实验设计 |
| 负结果不被最终论文掩盖 | 独立证伪、评审、归档位置 | 具备存放位置；需要执行记录保留政策 |
| 迁移可追溯 | 快照、旧新路径、SHA-256 | 检查器可验证快照与目标存在性 |
| 新资料能够确定下一步 | `99_待整理资料` 与归属规则 | 有入口；尚无负责人、期限及滞留监测 |
...
| `00_项目治理` | 集中维护规则、入口和结构声明 | 流程、布局、决策、迁移说明、目录审计 | 不承载模型科学结论；总表里的历史结论需独立标注 | 保留 |
| `01_models` | 将论证与原始假设绑定 | 六条路线及新增路线模板 | 六条不是互斥且穷尽的自然分类 | 保留主轴 |
| `02_共享基础` | 避免复用方法和工具出现多份主实现 | 协议、共同参考资料、共享源码 | 共享实现可能含理论前提，不能等同中立基准 | 保留并审查依赖 |
| `03_跨体系研究` | 跨模型判断与模型内部结论职责不同 | 公平比较、领域对照、综合候选、应用探索 | 应用并非严格的比较工作；现有内容少，暂作附属方向 | 有条件保留 |
| `04_公共成果` | 多模型综述不自然属于任一路线 | 综合报告、公共可视化 | 与单模型论文必须按研究范围区分；历史报告放这里不表示已发表 | 保留并明确范围 |
| `90_历史归档` | 保存跨项目历史及迁移证据 | 旧布局、旧工具、跨模型历史资料、快照 | 与模型内归档按影响范围区分；ZIP 不是长期仓储策略的全部 | 保留 |
| `99_待整理资料` | 避免为不明材料猜测归属 | 待立项问题、待审资料 | 必须有处置流程，否则会成为新的混乱源 | 保留并补管理 |
...
下表适用于六条路线和 `新体系模板`；全部实际实例另在逐目录附录登记。

| 目录 | 核心输入 → 输出 | 单独保留的理由 | 与相邻阶段的边界 |
|---|---|---|---|
| `00_研究立项` | 问题 → 范围、计划、退出条件 | 避免没有研究问题就堆积结果 | 计划不充当假设或证据 |
| `01_文献来源` | 原始来源 → 有出处的基准与差距 | 让借用理论和原创主张可区分 | 公共书目链接共享层，模型特有阅读判断留此 |
| `02_基础公设` | 宇宙认知 → 显式公设、适用域 | 每条证明必须能追溯到前提 | 定义符号不能代替本体假设 |
| `03_数学形式` | 公设 → 变量、单位、场空间和边界 | 同词不同义、度规或单位差异可在此消歧 | 演算过程进入推导 |
| `04_理论推导` | 公设与形式 → 方程和近似 | 保留公式的来源 | 得到方程不等于其稳定、因果或自洽 |
| `05_一致性检查` | 方程 → 条件性证明与一致性结果 | 将“导出”与“性质成立”分开 | 数学和内部检验不冒充观测证据 |
| `06_可检验预测` | 模型与参数 → 可测量差异及阈值 | 支持预先规定检验，减少事后解释 | 已知数据拟合必须注明，不能当独立预测 |
| `07_计算复现` | 算法配置 → 可重运行实现与日志 | 管理可执行研究资产 | 科学解释进验证或结论；主数据进入数据层 |
| `08_研究数据` | 来源 → 可追溯输入与派生数据 | 来源与处理历史需要独立管理 | 运行清单引用 data_id，不复制多份主数据 |
| `09_验证结果` | 预测、运行、独立数据 → 比较与残差 | 区分实现验证和观测检验 |误差预算在下一层，检验结果引用其版本 |
| `10_误差与不确定性` | 数据与方法误差 → 可信区间和稳健性 | 防止以很多有效数字代替可靠性 | 误差不能只在最终论文补写；参与前期设计 |
| `11_证伪与反例` | 预设阈值与反例 → 失效条件、负结果 | 确保不利结果有稳定入口 | 与验证使用同一主结果，通过链接组织 |
| `12_研究结论` | 全部证据 → 有条件的结论和开放问题 | 结论随证据版本演化 | 不能只从论文摘要回填“成功” |
| `13_论文与成果` | 已登记结论 → 正文、图表和补充材料 | 面向读者的叙述不同于研究工作区 | 论文不是唯一证据存储位置 |
| `14_独立评审` | 某个版本 → 意见、回复、独立复现 | 审查过程需可追踪且区别于作者自评 | 评审可以发生于任何阶段，不必等论文写完 |
| `15_版本发布` | 经审查版本 → 冻结发布清单与引用 | 让外部读者准确复现某次发布 | 发布清单不等于复制整个工作目录 |
| `90_历史归档` | 被替代/停止材料 → 历史链和原因 | 保留模型内演变与负结果 | 模型整体稳定入口仍保留，不因失败删除 |
...
| 子目录 | 判断依据与放置规则 |
|---|---|
| `07_计算复现/源码` | 模型专属可复用实现；阶段旁的历史入口可保留，但核心算法尽量只有一份 |
| `07_计算复现/交互笔记` | 交互探索、展示与试验；正式结果必须能重启后顺序复算 |
| `07_计算复现/配置` | 输入参数、单位、算法开关；与输出日志分离 |
| `07_计算复现/测试` | 自动化断言、解析极限、数值收敛；测试通过不等于模型获实验支持 |
| `07_计算复现/运行记录` | 每次执行的命令、环境、版本、日志和失败状态；输出大文件引用主存储 |
| `08_研究数据/原始数据` | 已接收的原始数据主副本，不覆盖；只读目前是约定，没有文件权限强制 |
| `08_研究数据/处理后数据` | 由可追溯流程生成的派生数据，登记输入和变换版本 |
| `08_研究数据/外部数据` | 外部来源元数据、许可和获取说明；与 原始数据 不重复保存同一主副本 |
| `13_论文与成果/论文正文` | 正文及参考文献；研究草稿与投稿版本须有版本标识 |
| `13_论文与成果/图表` | 图表及生成来源；若图像已为运行主输出，则引用该输出，避免手工不同步 |
| `13_论文与成果/补充材料` | 面向论文读者的补充说明和复现入口，不重复保管整套原始数据 |

### 7.2 共享、比较与历史子目录

`00_项目治理/资料索引` 提供派生视图，`维护工具` 保存索引生成程序；两者不是证据仓库。

`02_共享基础/参考资料` 保存共同参考资料，`研究规范` 保存方法约定，`公共计算/源码/三重奏统一场` 保存实际被多个模型调用的历史实现。当前共享源码没有在此次布局工作中补齐项目级依赖锁定、自动测试和独立基准，需要作为复现工程缺口登记。

`审计方法` 及 A1/A4/A5/A6 保留历史误差、自评和开放问题材料。特别是 `A6_AI科技星认证` 的名称不能证明第三方认证；A5 的历史分层与当前 `claims.csv` 证据类型应制定映射并解释差异。它们的历史正文未被本文重新科学审定。

`03_跨体系研究/比较方案` 存放比较方案与结果解释；父目录的 `comparison_matrix.csv` 作为跨比较总登记表。`物理领域` 提供引力、电磁、粒子等领域视图，`综合研究` 管理显式组合假设的综合候选，`应用研究` 管理应用可行性探索。后三者都不能跳过模型本身的证据链。

`04_公共成果/历史综合报告` 的四个系列子目录以历史作品系列分组，`可视化` 负责公共展示；这是一项兼容旧资料的折中。明确属于单模型的新论文应进入该模型，旧系列在完成出处和前提审查前不强行拆分。

根 `90_历史归档/历史资料` 下的 GAQ 各版本、旧布局，`历史工具` 的旧脚本，以及 `迁移记录/20260907_完整布局` 的快照和映射，分别解决研究史、工具史和迁移可恢复性三个问题。它们不应参与当前版本研究质量评分。

`99_待整理资料` 下十个问题目录只表示待处理主题。“大统一完成”“引力非重整化突破”等名字带有目标性语言，当前状态必须按待研究解释。建议在后续审理时采用中性的可检验问题标题，并保留旧名映射。
...
| 每模型只放 docs/源码/data/结果/papers | 小项目操作简单 | 公设、反例、误差和评审边界隐藏于文档内部 | 可能更适合早期轻量路线 |
...
| 中 | `99_待整理资料` 无处置期限，应用目录职责较宽 | 长期可能再次堆积 | 增加状态/负责人/下次处理日期；按实际工作量考虑拆分 |
...
- 审计生成器：[audit_directories.py](维护工具/audit_directories.py)，以全部实际目录为枚举对象。
